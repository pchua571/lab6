...
const binnedImages = async () => {
    const itemList = await client.hgetallAsync('binned');

    const items = [];

    for (let i of itemList) {
        const data = await client.getAsync(itemList[i]);
...
const userPostedImages = async () => {
    const userPostedList = await client.hgetallAsync('userpost');
...
const updateImage = async (
    ...
    if (userPosted || binned) {
        ...
        if (binned && !oldData.binned) {
            await client.rpushAsync('binned', id);
        }
